...
function embedlog(con, member, embed, message) {
  ...
  con.query(
    `SELECT EXISTS(SELECT log_channel FROM guild WHERE guildID = "${member.guild.id}")AS exist;`,
    (err, rows) => {
      let logchannel;
      if (err) console.log(err);
      if (rows[0].exist != null) {
        con.query(
          `SELECT log_channel AS channel FROM guild WHERE guildID = '${member.guild.id}';`,
          (err, rows) => {
            if (err) return console.log(err);
        ...
        for (let u = 0; u < lognames.length; u++) {
          logchannel = member.guild.channels.cache.find(
            (chan) => chan.name === lognames[u]
          );
          if (logchannel) {
            break;
          }
...
function embedless(con, member, embed, message) {
  con.query(
    `SELECT EXISTS(SELECT log_channel FROM guild WHERE guildID = "${message.guild.id}")AS exist;`,
    (err, rows) => {
      let logchannel;
      if (err) console.log(err);
      if (rows[0].exist != null) {
        con.query(
          `SELECT log_channel AS channel FROM guild WHERE guildID = '${message.guild.id}';`,
          (err, rows) => {
            if (err) return console.log(err);
      ...
      if (rows[0].exist != 0) {
        con.query(
          ...
          (err, rows) => {
            ...
            if (happening == 1) {
              logchannel.send({
                content: `ohno ${member.user.tag} left us ;_;`,
              });
              console.log(`member left:  ${member.user.tag}\n`);
            } else if (happening == 2) {
              logchannel.send({
                content: `${member.user.tag} joined us hooray !!`,
              });
        ...
        // Do nothing if the channel wasn't found on this server
        if (!logchannel) {
          console.log("no action taken no channel found");
        } else {
          if (happening == 1) {
