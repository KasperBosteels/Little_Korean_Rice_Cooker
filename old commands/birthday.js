...
module.exports = {
	...
	execute(client,message, args,con) {
        ...
        if (!user) {
		    return message.reply({content:'Please use a proper mention.'});
        }
        if(!channelID && !guildID) return message.reply({content:"Something went badly."});
        if(!args[1]){
        
            return message.reply({content:'You need to give me a date (DD/MM/YYYY) or "remove".'});
        }
        let data = verjaardag.GET(user.id);

        if(!regexTest(args[1].split('/').reverse().join('/')) && args[1] != "remove"){
            return message.reply({content:'DD/MM/YYYY is the right format.'});
        }
        if(args[1] == "remove"){
            if(data == false)return message.channel.send({content:'I have no birthday date of this person.'});
            if (!message.member.hasPermission("BAN_MEMBERS") && message.author.id != user.id){ 
                return message.channel.send({content:"Only moderators and the people this data is form, are allowed to delete it."});
            }
            ...
            return message.reply({content:'Removed the birthday.'});
        }
        if(regexTest(args[1].split('/').reverse().join('/'))){
            
            if(data != false) {
                return message.reply({content:'This users birthday is already set, please remove it first.'});
            }
            ...
            return message.channel.send({content:'Its saved.'});
        }
