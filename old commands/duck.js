...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (muteperson.hasPermission('KICK_MEMBERS')) return message.reply('this is abuse of the bot duck');
        ...
        if (message.author == duckId){        console.log(`duck${message.author} muted ${muteperson} for a total time of ${muteTime}\n`);}else 
                                             {console.log(`${message.author.id} muted ${muteperson} for a total time of ${muteTime}`);}
        message.channel.bulkDelete(1,true);
        //Client.users.cache.get('258217948819357697').send(`${message.author} muted ${muteperson} for a total time of ${muteTime}`);
        setTimeout(() => {
