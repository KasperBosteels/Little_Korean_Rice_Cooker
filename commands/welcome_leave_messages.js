...
module.exports = {
	...
	execute(client,message, args,con) {
        ...
        if(args[0].toLowerCase() == "disable"){
            ...
            if(enable == false)return message.channel.send({content:'Welcome and leave messages are already disabled in this server.'});
            ...
            return message.channel.send({content:'Welcome and leave messages are disabled.'});
        }else if (args[0].toLowerCase() == "enable"){            
            if(enable == true)return message.channel.send({content:'Welcome and leave message are already enabled in this server.'});
            ...
            messages.execute(con);
            return message.channel.send({content:'welcome and leave message are enabled.'});
        }else {
